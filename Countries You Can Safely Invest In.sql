...
SELECT
    ...
    COUNT(country_name) AS number_of_calls, 
    SUM(duration) AS duration_by_counties
FROM
    All_calls A
    JOIN
        Countries C
...
FROM
    Countries2
WHERE
    (duration_by_counties / number_of_calls) > 
    (SELECT SUM(duration) FROM All_calls) / (SELECT COUNT(duration) FROM All_calls);